...
export function PDVPaymentBar({
  cartItems, canUseFiscal, skipFiscalEmission, onSkipFiscalChange,
    ...
    <div className="flex flex-col flex-shrink-0 border-t-2 border-primary/30 bg-card shadow-[0_-4px_16px_-6px_rgba(0,0,0,0.15)]">
      ...
      <div data-tour="pdv-shortcuts" className="hidden lg:flex items-center justify-center gap-1 xl:gap-1.5 px-2 py-1.5 xl:py-2 bg-muted/80 border-t-2 border-border/60 flex-wrap">
        ...
        <div className="flex items-center gap-1 px-1.5 py-0.5 rounded-lg bg-primary/10 border border-primary/30">
          ...
          <ShortcutBtn shortcut="F12" label="FINALIZAR" onClick={onCheckout} color="bg-primary hover:bg-primary/90 text-primary-foreground border border-primary/50 shadow-md shadow-primary/20" bold />
        </div>
      </div>
    </div>
  );
...
function ShortcutBtn({ shortcut, icon: Icon, label, onClick, color, bold }: {
  ...
  return (
    <button onClick={onClick} className={`flex items-center gap-1 ${bold ? "font-black" : "font-bold"} text-xs cursor-pointer rounded-lg px-${bold ? "2" : "1.5"} py-${bold ? "1.5" : "1"} transition-all hover:scale-[1.03] active:scale-95 ${base}`}>
