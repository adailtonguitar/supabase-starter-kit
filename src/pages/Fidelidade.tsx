  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card><CardContent className="pt-6 text-center"><TrendingUp className="w-8 h-8 text-success mx-auto mb-2" /><p className="text-2xl font-bold">R$ {(topClients.reduce((s: number, c: any) => s + c.loyalty_points, 0) * Number(config?.redemption_value || 0.01)).toFixed(2)}</p><p className="text-xs text-muted-foreground">Valor em pontos (potencial)</p></CardContent></Card>
      </div>
      <Card>
