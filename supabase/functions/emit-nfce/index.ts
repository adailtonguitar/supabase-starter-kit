...
// ─── VALIDAÇÃO AUTOMÁTICA NCM por descrição (warning-only) ───
...
async function handleEmit(supabase: any, body: any) {
  ...
  const ambiente = config.environment === "producao" ? "producao" : "homologacao";
   ...
   for (let vi = 0; vi < items.length; vi++) {
     ...
     if (ncmCheck) {
       console.warn(`[NCM-VALIDACAO] Item ${vi + 1} "${itVal.name || itVal.xProd || ""}" — NCM atual: ${itVal.ncm}, sugestão: ${ncmCheck.sugestao} (${ncmCheck.desc})`);
     }
   }
  ...
  const ST_FALLBACK_NFCE: Record<string, Record<string, { mva: number; aliquotaInterna: number; cest?: string }>> = {
    ...
    "34022000": { _default: { mva: 43.53, aliquotaInterna: 18, cest: "2000300" }, MA: { mva: 43.53, aliquotaInterna: 22, cest: "2000300" } },
  };
  // Mapeamento NCM→CEST de último recurso NFC-e
  const CEST_FALLBACK_MAP_NFCE: Record<string, string> = {
    ...
    "3401": "2000300", "4011": "1600100", "8536": "1200100", "3402": "2000300",
    "8703": "0100100", "8704": "0100200", "8708": "0100500", "9619": "2000300",
...
async function handleEmitNfe(supabase: any, body: any) {
  ...
  const companyNeighborhood = pickFirstNonEmpty(company.neighborhood, company.address_neighborhood);
  const companyCity = pickFirstNonEmpty(company.city, company.address_city);
  const companyState = pickFirstNonEmpty(company.state, company.address_state).toUpperCase();
  const companyZip = onlyDigits(pickFirstNonEmpty(company.zip_code, company.address_zip, company.cep));
  ...
  for (let vi = 0; vi < items.length; vi++) {
    ...
    if (vNcm && vNcm.length === 8) {
      ...
      if (stCfg.temST) {
        ...
        if (stCfg.source === "none") {
          console.warn("[FISCAL-ST-FALLBACK]", { ncm: vNcm, uf: stUf, motivo: "sem_regra" });
          stLog.blocked = false;
          stLog.block_reason = null;
          stLog.risk_score = 0;
          stLog.motivo = `NCM ${vNcm} sem regra de ST — seguindo sem ST (fallback)`;
  ...
  const detItems = items.map((item: any, i: number) => {
    ...
    if (dynamicMatchNfe.rule || dynamicMatchNfe.score > 0) {
      console.log(`[FISCAL-RULE] NF-e item=${i + 1}`, JSON.stringify({
        ncm, regra_encontrada: dynamicMatchNfe.rule?.id || null,
