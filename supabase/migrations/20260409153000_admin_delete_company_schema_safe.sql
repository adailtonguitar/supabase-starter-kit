-- Make admin_delete_company resilient to schema drift:
-- delete only when table+column exists in current database.
...
COMMENT ON FUNCTION public.admin_delete_company(uuid, boolean) IS
  'Deletes demo/company records with schema-safe checks for table/column drift.';