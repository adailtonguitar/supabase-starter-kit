...
// ─── Pipeline com Aprendizado (assíncrona) ───

/**
 * Versão assíncrona do pipeline que integra o motor de aprendizado.
 * Requer companyId e supabase no contexto.
 * 
 * Fluxo por item:
 * 1. Engine calcula regra base
 * 2. Sistema consulta histórico + overrides
 * 3. Aplica ajustes conforme confiança
 * 4. Valida resultado final
 */
export async function runFiscalPipelineWithLearning(
  items: PipelineItemInput[],
  context: PipelineContext,
): Promise<PipelineResult> {
  // Se não tem dados de aprendizado, fallback para pipeline síncrona
  if (!context.companyId || !context.supabase) {
    return runFiscalPipeline(items, context);
  }
  ...
  const allWarnings: string[] = [];
  const processedItems: PipelineItemResult[] = [];
  const allLearningAdjustments: LearningAdjustment[] = [];
  ...
  for (const item of items) {
    ...
    try {
      learningResult = await applyLearning(context.supabase, {
        companyId: context.companyId,
        ncm,
        ufOrigem: context.ufEmitente,
        ufDestino: context.ufDestinatario,
        engineCfop: classification.cfopSugerido,
        engineCsosn: isSimples ? classification.cstOuCsosnSugerido : undefined,
        engineCst: !isSimples ? classification.cstOuCsosnSugerido : undefined,
        enginePisCst: undefined,
        engineCofinsCst: undefined,
        engineTemST: classification.temST,
        engineTemDifal: classification.temDifal,
      });
    } catch {
      // Falha no aprendizado não bloqueia
    }

    // ── Step 3b: Aplicar ajustes do aprendizado ──
    let adjustedCfop = classification.cfopSugerido;
    let adjustedCstCsosn = classification.cstOuCsosnSugerido;

    if (learningResult && learningResult.adjustments.length > 0) {
      for (const adj of learningResult.adjustments) {
        allLearningAdjustments.push(adj);

        if (adj.action === "apply") {
          learningApplied = true;
          if (adj.field === "cfop") adjustedCfop = adj.suggestedValue;
          if (adj.field === "csosn" && isSimples) adjustedCstCsosn = adj.suggestedValue;
          if (adj.field === "cst" && !isSimples) adjustedCstCsosn = adj.suggestedValue;
          allWarnings.push(`[LEARNING] ${adj.field}: "${adj.originalValue}" → "${adj.suggestedValue}" (confiança: ${(adj.confidence * 100).toFixed(0)}%)`);
        } else if (adj.action === "suggest") {
          allWarnings.push(`[SUGESTÃO] ${adj.field}: considerar "${adj.suggestedValue}" em vez de "${adj.originalValue}" (${adj.reason})`);
        }
      }
    }

    // ── Step 4: Calculate ICMS ──
    const icms = calculateIcms({
      valor: itemValor,
      crt: context.crt,
      ufOrigem: context.ufEmitente,
    ...
    // ── Step 6: Resolve final values (com ajustes do learning) ──
    const cfopFinal = adjustedCfop;
    const cstOuCsosn = adjustedCstCsosn || (isSimples ? icms.cstOuCsosn : icms.cstOuCsosn);
    ...
    processedItems.push({
      ...
      appliedRules: [
        ...
        ...icms.appliedRules,
        ...(learningResult?.source === "override" ? ["OVERRIDE_MANUAL"] : []),
        ...(learningResult?.source === "learning" ? ["APRENDIZADO_FISCAL"] : []),
      ],
    });
  }
  ...
  return {
    ...
    auditLog: {
      ...
      timestamp: new Date().toISOString(),
      learningApplied,
      learningAdjustments: allLearningAdjustments,
    },
  };
}
